...
/**
 * Import CSV eau -> table utility_readings (energy_type=water)
 *
 * Usage :
 *   php import_eau.php                         <- dry-run (aucune ecriture)
 *   php import_eau.php --execute [--user=<id>] <- import reel (defaut: 1er compte)
 *
 ...
 * Le fichier CSV doit être placé dans docs/meter_readings_export_water.csv
 * php /config/www/energyv2/app/scripts/import_eau.php
 *
 * NOTE : la valeur 0 m³ est acceptee — elle correspond a la mise en service du compteur.
 */
...
$colEau       = array_search('eau', $header, true);

if ($colTimestamp === false || $colEau === false) {
    echo '[FATAL] Colonnes attendues : "Timestamp", "Eau". Colonnes trouvees : ' . implode(', ', $header) . "\n";
    fclose($handle);
    exit(1);
}
...
$stmt = $pdo->prepare(
    "INSERT IGNORE INTO utility_readings (user_id, energy_type, reading_at, counter_m3)
     VALUES ({$userId}, 'water', :reading_at, :counter_m3)"
);
